...
@app.post("/analyze")
async def analyze_image(file: UploadFile = File(...), query: str = Form(...), max_new_tokens: int = Form(2048), save_response: bool = Form(False)):
    image = Image.open(io.BytesIO(await file.read())).convert('RGB')
    prediction = analyzer(image, query, max_new_tokens)
